...
//	console.log(cityIdx + " : " + date + " - " + weather + "-" + map.get(weather));
	...
	// 1. 현재 날짜에서 날씨가 일치하지 않으면 Push 
        Schedule.find({$and:[{"st_time": {"$gte": st_time, "$lt": ed_time}}, {"weather": {$ne: weatherVal}}, {"weather":{ $ne: "NONE"}}, {"loc": {"$eq": cityIdx}}]}, function(err, schedules){
		var userMap = {};
...
// LISTEN
exports.listen = function(app, Schedule, User)
{
	// 1. Set Url
